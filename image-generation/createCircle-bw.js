...
async function loadImages() {
    ...
    for (let i = 0; i < fileNames.length; i++) {
        let filepath = `${__dirname}\\tatvas\\${fileNames[i]}`;
...
(async () => {
    ...
    drawMainCircle(canvas, margin, "d");

    //drawSpiritEgg(canvas);

    await exportCanvasToImage(canvas, `bw${size}`);
})();
...
function drawSpiritCircle(canvas, margin) {
    ...
    draw231Web(canvas, radius);

    return radius;
}

function draw231Web(canvas, radius) {
    const size = canvas.height;
    const ctx = canvas.getContext('2d');
    ctx.save();
    ctx.translate(size/2, size/2);
    ...
    const angleOfSection = (Math.PI * 2 / 24);
    const angleOffset = -Math.PI / 2 + angleOfSection*2;
    const points = [];
    for (let i = 0; i < 22; i++) {
        let angle = angleOfSection * (i < 21 ? i : i+1) + angleOffset;
        points.push({
...
function drawSigils({ctx, center, sectionAngle, colors, margin, width, side = "q"}) {
    ...
    for (let i = 0; i < colors.length; i++) {
        ...
        if (i < colors.length-1) {
            ...
            ctx.drawImage(img,
                0, 0, img.width, img.height,
                -imgSize/2, -imgSize/2 - center + margin + width/2, imgSize, imgSize);
