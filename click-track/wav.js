...
function applyEnvelope(data, envelope) {

    const output = [];

    const duration = envelope.attack + envelope.decay + envelope.sustain + envelope.release;
    const percents = {
        attack: envelope.attack / duration,
        decay: envelope.decay / duration,
        sustain: envelope.sustain / duration,
        release: envelope.release / duration
    };

    if (!envelope.attackPercent) {
        envelope.attackPercent = 1;
    }

    if (!envelope.sustainPercent) {
        envelope.sustainPercent = 0.7;
    }

    for (let i = 0; i < data.length; i++) {
        const percent = i / data.length;
        let sectionPercent = 0, modPercent = 0;

        if (percent < percents.attack) {
            sectionPercent = percent / percents.attack;
            modPercent = sectionPercent; // move from 0 to 1
        }
        else if (percent < percents.attack + percents.decay) {
            sectionPercent = (percent - percents.attack) / percents.decay;
            modPercent = (1-sectionPercent) * (1 - envelope.sustainPercent) + envelope.sustainPercent; // move from 1 to sustainPercent
        }
        else if (percent < percents.attack + percents.decay + percents.sustain) {
            sectionPercent = (percent - percents.attack - percents.decay) / percents.sustain;
            modPercent = envelope.sustainPercent;
        }
        else if (percent < percents.attack + percents.decay + percents.sustain + percents.release ) {
            sectionPercent = (percent - percents.attack - percents.decay - percents.sustain) / percents.release;
            modPercent = (1-sectionPercent) * envelope.sustainPercent; // move from sustainPercent to 0
        }

        output[i] = data[i] * modPercent;
    }
    return output;
}
